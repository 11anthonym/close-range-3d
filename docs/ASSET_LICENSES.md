...
| MakeHuman core base mesh head subset | `public/assets/faces/makehuman-head.glb`, `public/assets/faces/makehuman-head-meshopt.glb` | [MakeHuman Community](https://github.com/makehumancommunity/makehuman/blob/master/makehuman/data/3dobjs/base.obj) | CC0 as an exported MakeHuman model | 2026-07-15 |
...
cut of the official core base mesh. The uncompressed `makehuman-head.glb` is the
small, server-safe runtime proof asset; `makehuman-head-meshopt.glb` preserves
the optimized archival variant. Four in-game head bases apply distinct
proportions and newly generated skin finishes to that topology. The game falls
back to its continuous procedural head if the runtime GLB cannot be loaded or
parsed.
